...
- the number of [events and readings](../../microservices/core/data/Ch-CoreData.md#events-and-readings) generated by device services and passing through core data
- the number of EdgeX micro services currently being monitored through the [system management service](../../microservices/system-management/Ch_SystemManagement.md)
...
!!! Info
    The metrics are provided via an associated [executor](../../microservices/system-management/executor/Ch_SysMgmtExecutor.md) feeding the system management agent telemtry data.  In the case of Docker, a Docker executor is capturing standard Docker stats and relaying them to the system management agent that in turn makes these available through its APIs to the GUI.
