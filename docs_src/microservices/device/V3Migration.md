...
!!! warning
    Updates to this migration guide for V3 are still in progress. Content/structure below is from V2, except for Device MQTT
...
A sync data is published to the topic `incoming/data/{device-name}/{source-name}` where:
...
Commands send to the device will be send on the topic `command/{device-name}/{command-name}/{method}/{uuid}` where:
