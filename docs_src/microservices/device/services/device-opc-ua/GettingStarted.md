...
The service name must match/refer to the target device service(`device-opc-ua`), and the profile name must match the device profile name(`OPCUA-Server`) from the previous steps.
