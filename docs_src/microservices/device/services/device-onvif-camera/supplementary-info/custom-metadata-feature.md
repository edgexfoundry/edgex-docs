...
```shell
curl --request PUT 'http://0.0.0.0:59882/api/{{api_version}}/device/name/<device name>/CustomMetadata' \
...
```shell
curl http://localhost:59882/api/{{api_version}}/device/name/<device name>/CustomMetadata | jq .
```
...
```shell
curl http://localhost:59882/api/{{api_version}}/device/name/<device name>/CustomMetadata?jsonObject=WwogICAgIkNvbG9yIiwKICAgICJMb2NhdGlvbiIKXQo= | jq .
...
```shell
curl --request PUT 'http://0.0.0.0:59882/api/{{api_version}}/device/name/<device name>/DeleteCustomMetadata' \
    --header 'Content-Type: application/json' \
    --data-raw '{
