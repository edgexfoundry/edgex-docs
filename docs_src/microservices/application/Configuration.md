...
|Writable Pipeline `UseTargetTypeOfByteArray` | false* | Indicates whether to treat the incoming data to the functions pipeline as []byte. If `true` the first function is the pipeline must be able to handle `[]byte` data. If `false` the incoming data is expected to be an EdgeX `models.Event` and thus the the first function is the pipeline must be able to handle an `Event` |
| Writable Pipeline `ExecutionOrder`                   | Depends <br />on profile<br /> used*                         | Indicates the functions and the order of the function to execute for the functions pipeline. Must be a comma separated list of built in function names that exist in the Writable.Pipeline.Functions section. Please refer to [App Service Configurable - Getting Started](../AppServiceConfigurable#getting-started)  section for more details |
| **Writable Pipeline Functions** | --- | This section defines which built in functions are available to use in `ExecutionOrder`and how they are configured. If a function is not listed here it can not be used. Only built in function can be listed and configured here. Please refer to [App Service Configurable - Available Functions](../AppServiceConfigurable#available-functions)  section for more details |
