...
- PKI private keys
  * Docker: PKI generated by standalone utility every boot. Distribution via secrets volume. (Compliant.)
  * Snaps: PKI generated by standalone utility every boot. Deployed to `$SNAP_DATA/secrets`. (Compliant.)
...
- Secret store per-service authentication tokens
  * Docker: Distribution via secrets volume, generated every boot. (Compliant.)
  * Snaps: Distribution via secrets volume, generated every boot. (Compliant.)
...
and cryptographic secrets,
cryptographic secrets should be preferred.
...
- The cryptographic secret is typically longer in bits than a plain-text secret.
...
2. Dynamic injection of secret into process environment space
   ...
   Existing examples of this functionality include `vaultenv`,
...
4. Distribution via a secrets volume
   ...
   EdgeX must therefore implement its own compatible secrets mechanism.
   ...
   * Docker:
   ...
   generally, `/run` on a Linux system is a `tmpfs` volume.
   For reasons of portability, `/tmp/edgex/secrets` has been chosen as a
   cross-platform alternative to `/run`:
   on the MacOS Docker implementation,
   `/tmp` is a white-listed shared host directory,
   while `/run` is not.
   For production hardening, `/tmp` should be replaced by `/run`
   where it appears in `docker-compose` files.
   If the container runtime in use does not support host fileystem bind mounts,
   then a separate Docker secrets volume should be created for each service.

   * Snaps:
   Note that due to an existing [bug](https://bugs.launchpad.net/snap-confine/+bug/1620442),
   `$XDG_RUNTIME_DIR` is set to a snap-specific path
   (`/run/user/1000/snap.`_$SNAP_NAME_`/`),
   but the directory is **neither created nor accessible to the snap**.
   Because it may be necessary to share several secrets outside of the snap
   --such as secret store tokens--
   it is desirable to place them in a location that can be mapped to
   a snap content interface, such as `$SNAP_DATA/secrets`.
   Although not a `tmpfs`, this is the best available option at this time.
   The snap bug should be escalated in priority.
